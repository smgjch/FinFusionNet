...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        projected_size = horizental_convoluted_size//2
        self.horizental_projection_2 = nn.Linear(horizental_convoluted_size, projected_size)
        ...
        self.vertical_projection = nn.Linear(v_feature_input_size, projected_size)
        
        concated_size = projected_size*3

        # print(f"projection size {concated_size}")
        self.cross_attention = nn.MultiheadAttention(embed_dim=self.enc_in, num_heads=3)

        self.encoder_layer = nn.TransformerEncoderLayer(d_model=concated_size, nhead=3, dim_feedforward=512)
    ...
    def forward(self, inputs,_x,y,_y):
        ...
        # print(f"shape of projected h_features {h_features.shape}, v_features  {v_features.shape}")

        # x = torch.cat([h_features,v_features],dim=1)

        # print(f"shape of v_features after projection {v_features.shape}")
        # Cross-attention between h_features and v_features
        h_attn_output, _ = self.cross_attention(inputs, inputs, inputs)
        h_attn_output = h_attn_output.view(self.batch_size,-1)
        # h_attn_output, _ = self.cross_attention(h_features, v_features, v_features)
        # attn_output = h_attn_output + v_attn_output
        # print(f"shape of attn_output {attn_output.shape}")

        x = torch.cat([v_features, h_attn_output], dim=1)
...
class Model_ha_(nn.Module):
    def __init__(self, configs):
        ...
        print(f"projection size {concated_size}")
        self.cross_attention = nn.MultiheadAttention(embed_dim=projected_size, num_heads=3)
    ...
    def forward(self, inputs,_x,y,_y):
        ...
        # print(f"shape of v_features after projection {v_features.shape}")
        # Cross-attention between h_features and v_features
        h_attn_output, _ = self.cross_attention(v_features, h_features, h_features)
        v_attn_output, _ = self.cross_attention(h_features, v_features, v_features)
        attn_output = h_attn_output + v_attn_output
        print(f"shape of attn_output {attn_output.shape}")

        x = torch.cat([h_features,v_features, attn_output], dim=1)
        print(f"shape of x after cat {x.shape}")
