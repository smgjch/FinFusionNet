#D for deliation, T for transfomer, M for m sampling, _m for mix projection
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.horizental_projection_2 = nn.Linear(self.enc_in, 1)
        ...
        concated_size = input_size + self.enc_in*self.input_window_size
        v_projection_size = concated_size//5+2

        self.vertical_projection = nn.Linear(concated_size, v_projection_size)
        print(f"projection size {v_projection_size}")

        self.encoder_layer = nn.TransformerEncoderLayer(d_model=v_projection_size, nhead=3, dim_feedforward=512)
        self.transformer_encoder = nn.TransformerEncoder(self.encoder_layer, num_layers=4)
        self.output_layer = nn.Linear(v_projection_size, self.label_len)
    ...
    def forward(self, inputs,_x,y,_y):
        ...
        x = F.relu(self.vertical_projection(x))
